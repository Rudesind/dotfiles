...
## Requirements
---
Obviously the only primary requirement is that you install the software of the file you wish to use.

Most of the configuration files are not specific to a single operating system, but note that you may need to make slight modifications to have them work correctly, and the location of the file may change from system to system. This is especially true if the managed software isn't supported on a specific operating system.

In these instances, I create an entirely separate branch for managing configuration on that operating system. Currently I manage a separate branch for the following operating systems:
...
## Included Software
---
I maintain specific configurations for the following software, or the following software is required to fully use my dotfiles:
...
---
I maintain my dotfiles using the **Bare Git Repository** method. This was inspired by [DistroTube](https://www.youtube.com/channel/UCVls1GmFKf6WlTraIb_IaJg). For creating you're own dotfiles using this method, see: https://www.atlassian.com/git/tutorials/dotfiles.

## Setup and Usage
---
I highly recommended that you review the guide for setting up *dotfiles* from [Atlassian](https://www.atlassian.com/git/tutorials/dotfiles), but I provide a summarized version with some of my own additions.
...
If you are installing on **Mac** using **Fish** or **Zsh** via [Homebrew](https://brew.sh/), you may need to make some alterations to the alias, as `git` is in the `homebrew` directory:
...
On **Windows** using **PowerShell**, it requires slightly different steps as aliases don't work quite the same. Instead, we need to create an entirely new function and alias to use `config`.
...
After we've created our alias, we want to hide untracked files. This will be helpful as running `config status` won't show a long list of we aren't interested in tracking. Since our working tree is set to our `$HOME` directory, running `status` will show files in our `$HOME` directory.
...
You will need to backup (optional) and delete the files before the repository can be checked out. This will replace the deleted files by a managed copy from the repository.
...
Before we checkout the repository, we need to create a new branch. Please ensure the alias has been defined as seen in the **Initializing New Repository** section, then run the following command:
...
### Changing Terminal to Fish on Mac
After installing my dotfiles you might notice that iTerm still doesn't appear correctly. You'll need to change your shell to fish using the following commands:

```bash
sudo sh -c 'echo /opt/homebrew/bin/fish >> /etc/shells'
```

Restart the terminal, then run this command:

```bash
chsh -s /opt/homebrew/bin/fish
```

Afterwards the terminal should start with the correct shell.
